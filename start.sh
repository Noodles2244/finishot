...
echo -e "${BLUE}[1/4] Cloning Silly Proxy repository...${RESET}"
...
echo -e "${BLUE}[3/4] Starting Silly Proxy...${RESET}"
npm start

if [ $? -eq 0 ]; then
    echo -e "${GREEN}[Success] Silly Proxy started successfully.${RESET}"
    echo -e "${YELLOW}[Info] You can access Doge V4 via localhost.${RESET}"
else
    echo -e "${RED}[Error] Failed to start Silly Proxy. Please check the application logs for details.${RESET}"
    exit 1
fi
